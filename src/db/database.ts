...
// PICK-SIDE probability (max(calibrated_prob, 1-calibrated_prob)) so a 35%
// home pick counts as a 65% away pick — that way buckets always live in
// [0.5, 1.0] regardless of which side the model favored. Buckets are
// half-open [lo, hi). Only buckets with at least one graded pick are
// returned so the embed stays compact early in the season.
export function getConfidenceBuckets(season: string): ConfidenceBucket[] {
  const rows = queryAll<{ calibrated_prob: number; correct: number }>(
    `SELECT calibrated_prob, correct FROM predictions
     WHERE game_date LIKE ? AND correct IS NOT NULL`,
    [`${season.split('-')[0]}-%`]
  );
